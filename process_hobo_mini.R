...
pathRawData=paste0(getwd(),
                   '/../Avenelles/raw_data/HOBO_data/')
#path to store processed data
pathProcessedData=paste0(getwd(),'/../Avenelles/processed_data_KC')

# listHobo contient la liste de tous les dossiers dans raw_data/HOBO_data
# ces Hobos sont à la fois ceux des LOMOS et ceux des points HZ
listHobo = list.files(pathRawData)[!grepl('invalid',list.files(pathRawData))]

# get names of Hobo
# namesHobosList contient la liste des Hobos utilises pour les stations LOMOS
namesHobosList = read.csv(file = '../Avenelles/raw_data/DESC_data/DATA_SENSOR/pointNamesHobo.txt',
                          header=F,colClasses = 'character')[,1]
# namesHobo contient le nom du Hobo si il est utilise pour un LOMOS, 'HZ' sinon
namesHobo = rep('',length=length(listHobo))
for(i in 1:length(listHobo)){
  for(j in 1:length(namesHobosList)){
    if(grepl(namesHobosList[j],listHobo[i])){
      namesHobo[i] <- namesHobosList[j]
    }
  }
  if (namesHobo[i]==''){namesHobo[i]<-'HZ'}
}

# get LOMOS stations of hobos
NameStationCsv=read.csv(
  file = paste0(getwd(),
                '/../Avenelles/raw_data/DESC_data/DATA_STATION/dispoParStation.csv'),
  colClasses = 'character',header=T,sep=';')
# StationNames[i] contains LOMOS station name of namesHobo[i]
StationNames = character(length = length(namesHobo))
for (i in 1:length(namesHobo)){
  bool=F
  for (j in 6:11){
    Ind = which(namesHobo[i] == NameStationCsv[,j])
    if(length(Ind)>0){
      bool=T
      StationNames[i] = NameStationCsv[Ind,1]
    }
  }
  if(!bool){StationNames[i]='HZ'}
}
...
for (i in 1:length(listHobo)){
  ...
  # premiere boucle pour exclure les hobo des MOLONARI perennes
    ...
    dataHobo_0 = read.csv(fileHobo,skip=1,colClasses = 'character',sep=',',strip.white = T)
  ...
  for (j in 1:length(listFiles)){
    ...
    temperature_Hobo = data.matrix(dataHobo_0[,grepl('Temp',names(dataHobo_0))])
    pression_Hobo = data.matrix(dataHobo_0[,grepl('Volt',names(dataHobo_0)) | grepl('Tension',names(dataHobo_0))])
    if(ncol(pression_Hobo)>0){ # if there are pressure differential measurements
      dataHobo[idxToFill,1] <- pression_Hobo
      if(ncol(temperature_Hobo)>0){ # in that case it is temperature in the bottom of the river
...
if(boolPlotPerDevice){
  for (i in 1:length(listHobo)){
    if (substring(listHobo[i],1,3)!="Hob") {
    png(file = paste0(getwd(),'/../plots/PerDevice/Hobo/RAW',listHobo[i],'.png'),
...
for (i in 1:length(listHobo)){
  ...
  if (StationNames[i]!='HZ'){ # if the Hobo is in a Molonari take only temperature meas
    write.table(dataMatrix[,c(1:2,5:8,13)],
                file=paste0(pathExportStation,listHobo[i],'.csv'),
                dec='.',row.names=F,sep=',',quote = F)
  }else{
    if(!file.exists(paste0(pathExportStation,listHobo[i]))){
      dir.create(paste0(pathExportStation,listHobo[i]))
    }
    ...
    if(sum(c(3,4) %in% whichColsNonEmpty) > 0){
      write.table(dataMatrix[,c(1:4,13)],
                  file=paste0(pathExportStation,listHobo[i],'/p_',listHobo[i],'.csv'),
                  dec='.',row.names=F,sep=',',quote = F)
    }
    ...
    if(sum(5:10 %in% whichColsNonEmpty) > 0){
      write.table(dataMatrix[,intersect(whichColsNonEmpty,c(1:2,5:10,13))],
                  file=paste0(pathExportStation,listHobo[i],'/t_',listHobo[i],'.csv'),
                  dec='.',row.names=F,sep=',',quote = F)
    }
  }
  }
